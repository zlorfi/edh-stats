	...
	<div class="min-h-screen bg-gray-50">
		...
		<main class="container mx-auto px-4 py-8">
			<div class="max-w-2xl mx-auto">
				<h1 class="text-3xl font-bold text-gray-900 mb-8 text-center">Round Counter</h1>
				
				<div class="card">
					<!-- Round Display -->
					<div class="text-center mb-8">
						<p class="text-2xl text-gray-600 mb-2">Current Round</p>
						<p class="text-8xl font-bold text-indigo-600 mb-6">{currentRound}</p>
						<button
							on:click={nextRound}
					...
					<div class="text-center mb-8 border-t pt-8">
						...
						<p class="text-5xl font-bold text-gray-900 font-mono mb-4">{elapsedTime}</p>
						<p class="text-lg text-gray-600">
							Average per round: <span class="font-semibold">{avgTimePerRound}</span>
						</p>
					</div>
					...
					<div class="flex gap-4 mb-6">
						{#if !counterActive && !hasPausedGame}
							<button on:click={startCounter} class="flex-1 btn btn-primary text-lg py-3">
								▶️ Start Counter
							</button>
						{:else if counterActive}
							<button on:click={stopCounter} class="flex-1 btn btn-secondary text-lg py-3">
								⏸️ Pause
							</button>
				...
				<!-- Instructions -->
				<div class="card mt-6">
					<h2 class="text-lg font-bold mb-3">How to Use</h2>
					<ol class="list-decimal list-inside space-y-2 text-sm text-gray-700">
						<li>Click "Start Counter" to begin tracking your game</li>
						<li>Click "Next Round" each time a new round begins</li>
						<li>Use "Pause" if you need to take a break</li>
						<li>When finished, click "End Game & Log Results" to save the game</li>
						<li>The counter will remember your game if you navigate away</li>
					</ol>
				</div>
			</div>
		</main>
